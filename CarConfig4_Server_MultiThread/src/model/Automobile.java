...
{   
	...
	public synchronized ArrayList<String> getOptionNamesinSet(String setName){
		ArrayList <String> optionNames= new ArrayList<String>();
		for (int i = 0; i< this.findOptionSet(setName).getOptArray().size(); i++){
			optionNames.add(this.findOptionSet(setName).getOptArray().get(i).getName());
		}
		return optionNames;
	}
...
//		notify();
    ...
    public String printChoice(){
    	...
    	for (int i=0;i<this.getOptionSets().size();i++){
    		String setName=this.getOptionSets().get(i).getName();
    		str=str+setName+":"+this.getOptionChoice(setName)+"\n";
    	}
    	return str;
    }
